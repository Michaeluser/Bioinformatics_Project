...
P_DVC="$HOME/Data/Variant_Calling/SNP_Results/P_DVC.vcf.gz" #primary derived variant calls
...
if [ "$1" == "-s" ]; then
	...
	if [[ "$2" == *"m"*  || "$2" == *"a"* ]]; then
		...
		mkdir -p $HOME/Data/Data_QC/ $HOMEData/Read_Alignment/ $HOME/Data/Post_Processing $HOME/Data/Post_Alignment_QC/ $HOME/Data/Variant_Calling/ $HOME/Data/? \
		$HOME/Data/Data_QC/Reports/ $HOME/Data/Read_Alignment/Reports/ $HOME/Data/Post_Processing/Reports/ $HOME/Data/Post_Alignment_QC/Reports/ $HOME/Data/Variant_Calling/CNV_Results/ $HOME/Data/Variant_Calling/Manta_Output/ $HOME/Data/Gleaning_Annotation/
	...
	if [[ "$2" == *"d" || "$2" == *"a"* ]]; then
		...
		#install tools
		for tool in seqtk seqkit samtools bcftools fastqc bwa fastp qualimap gatk4 cnvkit igv survivor; do
			install_tool "$tool"
		done
	fi
...
bcftools sort $CVR -Oz -o $CNV_VC

#index vcf file
#t - create a TBI index
bcftools index -t $CNV_VC
